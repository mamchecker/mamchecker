...
from mamchecker.model import (Student,Problem,Assignment,
        studentCtx,delete_all,copy_all,ctxkey,keyparams)
import logging
    ...
    def post_response(self):
        ...
        if choice != '0':#not new
            ...
            if choice == '1' and pathchanged or choice == '2':#delete
                ...
                newname = '/'.join([v for k,v in self.request.student.key.pairs()])
                oldstudent.key.delete()
                #no student any more, redirect to get/generate a new one
                if choice == '1':#change
                    self.redirect("message?msg=h&oldname={}&newname={}".format(oldname,newname))
                elif choice == '2':#delete
                    self.redirect("message?msg=g&studentname={}".format(oldname))
                return
