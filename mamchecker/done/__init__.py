...
def prepare(
        ...
        ,userkey #if a parent belongs to user then all children can be queried
        ):
        ...
        for le in x.split(','):
            ...
            if match:
                ...
                if name in AGES.values():
                    age = name
                if age:
                    value = datetime.datetime.now()-datetime.timedelta(**{age:int(value)})
                    name = 'answered'
...
class Page(PageBase):
    ...
    def __init__(self, _request):
        ...
        self.table = lambda: depth_1st(*prepare(self.request.query_string, 
            self.request.student.key,self.user and self.user.key))
        self.params = {
            'table': self.table,
