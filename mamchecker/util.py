...
class Util:
    ...
    def query(self):
        return [d[0] if not d[1] else d for d in parse_qsl(self.request.query_string,True)]
    def a(self,lnk):
        return '<a href="/'+self.request.lang+'/?'+lnk+'">'+lnk+'</a>'
    def newlang(self,lng):
        np = self.request.path_qs.replace('/'+self.request.lang,'/'+lng,1)
    ...
    @staticmethod
    def all_of(user):
        students = Student.query(Student.userkey == user.key).iter(keys_only=True)
...
class PageBase(AuthUser):
    ...
    def get_response(self):
        return template(self.request.pagename
                ,self.params
                ,template_lookup=mklookup(self.request.lang)
                )

    def redirect(self, afterlang):
        return webapp2.redirect('/{}/{}'.format(self.request.lang,afterlang)
